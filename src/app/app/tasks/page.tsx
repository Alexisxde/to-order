"use client"
import Column from "@/components/tasks/desktop/column"
import DeleteCard from "@/components/tasks/desktop/delete-card"
import FormCreateTask from "@/components/tasks/form-create-task"
import Tab from "@/components/tasks/mobile/tab"
	...
	return isMobile ? (
		...
		<section className="flex gap-4 p-4">
			{COLUMNS.map(({ title, column, textColor, bgColor }) => (
				<Column key={column} title={title} column={column} textColor={textColor} bgColor={bgColor} />
			))}
			<div className="fixed right-4 bottom-4">
				<MorphingDialog isOpen={isOpen} setIsOpen={setIsOpen}>
					...
					<MorphingDialogContainer overlay={false} className="items-end justify-end pr-4 pb-5">
